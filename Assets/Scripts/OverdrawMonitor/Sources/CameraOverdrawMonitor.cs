...
public class CameraOverdrawMonitor : MonoBehaviour
{
    ...
    public float showLocalOverdrawRatio = 0;
    public float showMaxLocalOverdrawRatio = 0;
    public float showGlocalOverdrawRatio = 0;
    public float showMaxGlocalOverdrawRatio = 0;
#if UNITY_EDITOR
    void OnGUI()
    {
        var resString = UnityEditor.UnityStats.screenRes.Split('x');
        int x = int.Parse(resString[0]), y = int.Parse(resString[1]);
        ...
        GUI.Label(new Rect(50, 50, 400, 60), $"{x} x {y} : {showLocalOverdrawRatio} - {showMaxLocalOverdrawRatio}", style);
    }
