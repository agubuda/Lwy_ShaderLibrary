    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(a2v inside)
            {
                ...
                o.positionCS = TransformObjectToHClip(inside.vertex.xyz);
            ...
            float4 frag(v2f inside) : SV_TARGET
            {
                ...
                float3 ShiftT = inside.worlBbitangent + inside.worldNormal * noiseMap.xyz * _NoisePower;
                ...
                //depth tex
                // float2 screenPos = inside.screenPos.xy / inside.screenPos.w;
                // float4 depthTex = SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, screenPos);

                float4 finalColorPhong = saturate(anistropy + _BaseColor * remapLambert * float4(mlight.color, 1) * diffuseColor +phongSpecular);
